...
class Chamber:
    ...
    def __init__(
        ...
        x_candidates = [
            x for x in range(self.x_min + 1, self.x_max) if x not in x_to_avoid
        ]
        y_candidates = [
            y for y in range(self.y_min + 1, self.y_max) if y not in y_to_avoid
        ]
    ...
    def _build_dividing_walls(
        ...
        for col in cells[self.x_min:self.x_max + 1]:
            try:
                ...
                print(e)
                print(f"Index: {self.center_y}, len(col): {len(col)}")
    ...
    def _open_doors(
        ...
        def get_walls_and_doors(
            wall_from: int, wall_to: int, door_x: int, door_y: int, is_open: bool
        ) -> Wall:
        ...
        walls_and_doors = {
            "v_upper_wall": get_walls_and_doors(
                ...
                self.center_y - 1,
                self.center_x,
                randint(self.y_min, self.center_y - 1),
            ...
            "v_lower_wall": get_walls_and_doors(
                ...
                self.y_max,
                self.center_x,
                randint(self.center_y + 1, self.y_max),
                "v_lower_wall" == closed_wall,
            ),
            "h_left_wall": get_walls_and_doors(
                self.x_min,
                self.center_x - 1,
                randint(self.x_min, self.center_x - 1),
                self.center_y,
            ...
            "h_right_wall": get_walls_and_doors(
                self.center_x + 1,
                self.x_max,
                randint(self.center_x + 1, self.x_max),
                self.center_y,
    ...
    def _build_sub_chambers(self, walls_and_doors: dict[str, Wall]) -> list["Chamber"]:
        sub_chambers = []
        for v_w in ["v_upper_wall", "v_lower_wall"]:
            for h_w in ["h_left_wall", "h_right_wall"]:
                ...
                if (x_max - x_min >= 3) and (y_max - y_min >= 3):
                    ...
                    sub_chamber = Chamber(
                        x_min, x_max, y_min, y_max, x_to_avoid, y_to_avoid
                    )
